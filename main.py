...
from db.referencia_db import get_referencia
from models.hotel_models import HotelIn, HotelOut
from models.referencia_models import RefIn, RefOut, TempOut,CostOut
...
@app.get("/ref/search/{nombre}")
async def get_ref(nombre: str):
    ref_in_db = get_referencia(nombre)
    if ref_in_db == None:
        raise HTTPException(status_code=404, detail="EL REGISTRO NO EXISTE EN LA BASE DE DATOS")
    ref_out = RefOut(**ref_in_db.dict())
    return ref_out
